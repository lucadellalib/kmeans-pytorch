...
class KMeansResidualQuantizer(KMeansMultiQuantizer):
    ...
    def reset_parameters(
        self, feats: "Tensor", index: "Optional[int]" = None
    ) -> "None":
        assert feats.shape[-1] == 1
        if index is not None:
            self.kmeanss[index].reset_parameters(feats[..., 0])
            return
        for i, kmeans in enumerate(self.kmeanss):
            kmeans.reset_parameters(feats[..., 0])

    def forward(
        self,
        feats: "Tensor",
        return_centroids: "bool" = True,
        sum_centroids: "bool" = True,
        ...
        residual = feats[..., 0]
        for i, kmeans in enumerate(self.kmeanss):
            labels, assigned_centroids = kmeans(residual, return_centroids=True)
            labels_list.append(labels)
            assigned_centroids_list.append(assigned_centroids)
            if i == index or i == (len(self.kmeanss) - 1):
                break
            residual = residual - assigned_centroids
        labels = torch.stack(labels_list).movedim(0, -1)
        if return_centroids:
            assigned_centroids = torch.stack(assigned_centroids_list).movedim(0, -1)
            if sum_centroids:
                assigned_centroids = assigned_centroids.sum(dim=-1, keepdim=True)
            return labels, assigned_centroids
        return labels
    ...
    def step(
        ...
        for i, kmeans in enumerate(self.kmeanss):
            if i == index or i == (len(self.kmeanss) - 1):
                total_drift = kmeans.step(
                    residual,
                    labels[..., i] if labels is not None else None,
                    return_drift,
                )
                break
            _, assigned_centroids = kmeans(residual, return_centroids=True)
            residual = residual - assigned_centroids
        if return_drift:
            return total_drift / (i + 1)
    ...
    def evaluate(
        ...
        for i, kmeans in enumerate(self.kmeanss):
            inertia = kmeans.evaluate(
                residual, labels[..., i] if labels is not None else None
            )
            total_inertia += inertia
            if i == index or i == (len(self.kmeanss) - 1):
                break
...
def _compute_inertia(
    feats: "Tensor", centroids: "Tensor", labels: "Tensor",
) -> "Tensor":
...
if __name__ == "__main__":
    ...
    ax.plot(
        X[identical, 0], X[identical, 1], "w", markerfacecolor="#bbbbbb", marker=".",
    )
...
if __name__ == "__main__":
    quantizer = KMeansResidualQuantizer(num_features=300, num_clusters=[10, 10])
    input = torch.randn(5, 200, 300, 1)
    labels, centroids = quantizer(input)
    print(labels.shape)
    print(centroids.shape)
    print(quantizer.centroids.shape)
